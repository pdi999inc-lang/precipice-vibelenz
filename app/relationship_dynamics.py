...
class RelationshipAnalyzer:
    ...
    def analyze(self, turn_history: List[Dict]) -> Optional[RelationshipInsight]:
        ...
        if not turn_history or len(turn_history) < MIN_TURNS:
            logger.info(f"RelationshipAnalyzer: insufficient turns ({len(turn_history) if turn_history else 0} < {MIN_TURNS})")
            return None

        try:
            return self._analyze(turn_history)
        except Exception as e:
            logger.error(f"RelationshipAnalyzer: analysis failed — {e}")
