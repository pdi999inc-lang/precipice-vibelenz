...
async def analyze_screenshots(
    ...
    # --- Hard downgrade for false-positive grooming in relationship lanes ---
    if relationship_type in {"dating", "family", "friend", "business"}:
        phase = str(result.get("phase") or "").upper()
        flags = [str(x).lower() for x in (result.get("flags") or [])]
        active_combos = " ".join(str(x).lower() for x in (result.get("active_combos") or []))
        evidence = result.get("evidence") or {}
        evidence_text = ""
        if isinstance(evidence, dict):
            evidence_text = " ".join(str(v).lower() for v in evidence.values())
        else:
            evidence_text = str(evidence).lower()
        summary_text = str(result.get("summary") or "").lower()

        combined = " ".join(flags) + " " + active_combos + " " + evidence_text + " " + summary_text + " " + phase.lower()

        hard_terms = [
            "minor", "underage", "age gap", "power imbalance",
            "coerc", "threat", "blackmail", "isolation", "secrecy",
            "exploit", "conditioning", "dependency", "repeated manipulation",
            "money", "gift card", "wire", "cashapp", "venmo", "paypal", "bitcoin", "crypto"
        ]
        ...
        if phase == "GROOMING" and not any(term in combined for term in hard_terms):
            ...
            result["flags"] = ["uncertain_identity", "rapid_flirtation", "needs_verification"]
            result["active_combos"] = []
            result["summary"] = "Conversation shows identity ambiguity and quick flirtation escalation, but no clear coercive, exploitative, or financial manipulation in the visible exchange."
    ...
    if result["risk_score"] >= 70:
        ...
        risk_label = "Medium"
# FINAL OUTPUT SANITIZATION (prevents false grooming display)
if relationship_type in {"dating", "family", "friend", "business"}:
    if str(result.get("phase", "")).upper() == "GROOMING":
        result["phase"] = "NONE"

    result["flags"] = [
        x for x in result.get("flags", [])
        if "groom" not in str(x).lower()
    ]

    result["summary"] = str(result.get("summary", "")).replace("groom", "").replace("GROOM", "")
